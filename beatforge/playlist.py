...
class PlaylistManager:
    ...
    #     """
    #     Persiste a lista de TrackDTO em CSV, incluindo um campo 'features_json'
    #     que armazena todo o dicionário de features serializado em JSON.
    #     """
    #     if csv_path is None:
    #         csv_path = f"{config.FILENAME}.csv"

    #     # Adicionamos 'features_json' ao cabeçalho
    #     fieldnames = [
    ...
    #     """
    #     Carrega TrackDTO a partir de um CSV que contém o campo 'features_json'.
    #     Desserializa cada linha em um TrackDTO, convertendo o JSON de volta para dict.
    #     """
    #     if csv_path is None:
    #         csv_path = f"{config.FILENAME}.csv"
    ...
    def save_tracks_db(self, tracks: List[TrackDTO], db_path: Optional[str] = None) -> None:
        """
        Persiste a lista de TrackDTO em SQLite, criando sempre a tabela 'track_info'
        com o campo 'features_json' e gravando o dicionário de features como JSON.
        """
        if db_path is None:
        ...
        # Cria a tabela com todas as colunas, incluindo 'features_json'
        cur.execute("""
    ...
    def load_tracks_db(self, db_path: Optional[str] = None) -> List[TrackDTO]:
        """
        Carrega TrackDTOs do banco SQLite, incluindo a desserialização do campo 'features_json'
        para reconstruir o dicionário de features em cada objeto.
        """
        if db_path is None:
            db_path = f"{config.FILENAME}.db"
        ...
        for row in cur.execute("""
            ...
            FROM track_info
        """):
            raw_json = row[9]  # índice referente a 'features_json'
            try:
                feats: Dict[str, Any] = json.loads(raw_json or "{}")
            except json.JSONDecodeError:
                feats = {}
